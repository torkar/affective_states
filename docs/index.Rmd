...
So for each linear model in our multivariate model we have the following priors:
...
In short, the above shows how our combined priors look like in the probability space ($y$). Due to the gap in the middle one could say we are skeptical of any extreme curves around 0. In the end, prior predictive checks will allow us to investigate this on the outcome scale.

Above we used a Dirichlet prior on ordered categorical data. The Dirichlet is a generalization of the Beta distribution, probability (0,1), but instead of having one probability being estimated using a Dirichlet prior, we can have many. How many depends on the number of categories $K$, which means we need to set the prior with $K-1$ parameters (the last category we can calculate from the previous category).
...
So all of the above are for estimating $6+1$ categories using $6$ parameters. It's evident that the higher numbers we set for our parameters ($2 \rightarrow 4 \rightarrow 6$) the more mass is on lower probabilities. Having 2's implies an almost flat prior, i.e., we really don't have any good prior knowledge on how this simplex would look like so this is it.

# Model comparisons and prior predictive checks
Since we have a cumulative likelihood we'll have a hard time using any principled ways of conducting variable selection. Luckily, in our case, we are really not interested in arriving to as small a set of predictors as possible. We use all predictors and instead we do model comparisons while adding them one by one.

First, we executed one model at a time, and then we used LOO to compare their relative out of sampled predictions capabilities. We will not run all models below but simply list them here for completeness.
...
m8 <- brm(mvbind(D,A,V) ~ mo(EDU) + EXAMPLE_idx + MAJOR_idx + ROLE_idx +
            LANG_idx + ENTITIES + EXP + (1|c|ID_idx),
         data=dat, family=cumulative)
```

And here are our final priors and model $\mathcal{M}$, which we'll now run and sample only from the priors first. The model consists of all predictors, and in the case of EDU it's modeled as monotonic, i.e., ordered categorical with a Dirichlet prior. Additionally, for the varying intercept (ID_idx) we also model the correlation between our three outcomes $\{V,A,D\}$, the logic being that it's the same subject who's answered all question and thus it's likely that the answers are correlated. For the correlation we'll us the LKJ(2) prior (which can be considered a default prior for modeling correlations). The 2 in LKJ(2) implies that we are slightly skeptical of extreme correlations.

First, set priors on our 136 parameters we want to estimate and then sample only from the priors. We set the Lewandowski-Kurowicka-Joe (LKJ) prior for our correlations, $\mathcal{N}(0, 0.3)$ on $\beta$, and $\mathcal{N}(0, 2.5)$ on $\alpha$ (since we have a cumulative model we have several $\alpha$, one for each border between responses, i.e., $9$ responses means $8$ $\alpha$). We have $\mathrm{Weibull}(2,1)$ as prior for modeling $\sigma$ (allowing only positive real numbers) and, finally, our Dirichlet prior on the predictor EDU, which is ordered categorical. We recommend you to change the priors and see how this affects the prior probability distribution.
...
p$prior[2] <- "lkj(2)"
p$prior[c(5,49,93)] <- "normal(0,0.3)"
p$prior[c(36,80,124)] <- "normal(0,2.5)"
p$prior[c(45,89,133)] <- "weibull(2,1)"
p$prior[c(48,92,136)] <- "dirichlet(2,2,2,2,2)"

M_prio <- brm(mvbind(D,A,V) ~ mo(EDU) + EXAMPLE_idx + MAJOR_idx + ROLE_idx + 
                LANG_idx + ENTITIES + EXP + (1|c|ID_idx), prior = p, data=dat, 
         family=cumulative, sample_prior = "only", seed = SEED)
```

Now plot our priors in the outcome scale,

```{r cache=TRUE}
pp_check(M_prio, type="bars", resp = "V", nsamples = 1000)
```

The $y$-axis are the counts while the $x$-axis are the Likert scale outcome for the response $V$. As we see, the medians for our prior samples ($y_{\mathrm{rep}}$) are evenly distributed and the credible intervals are wide (the bars are our empirical data $y$). This looks to be a good setup for our model. Now, let's use our data and sample,
...
A minimum of three diagnostics should preferably always be checked before one puts any trust in a model (and you should never trust a model completely...) First we can use `plot(M)` to visualize how well the chains have mixed (they should look like fat hairy caterpillar plots). We won't plot them here since they will take up a lot of space.
...
```{r}
# the correlations give an NA, as they should so exclude it
...
So, diagnostics look good. Let's als o do some posterior predictive checks.
```{r}
