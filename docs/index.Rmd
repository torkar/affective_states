...
First, we executed one model at a time, and then we used LOO to compare their relative out of sampled predictions capabilities.
...
In the manuscript we report $95$% credible intervals, but here we can take the opportunity and do it properly and look at $89$% credible intervals. After all, $89$ is at least a prime number$\ldots$

The following parameters were significant on the $89$%-level:
...
We could also set some fixed values for our covariates and see what this implies. Below we define a function to do jus that,
```{r}
